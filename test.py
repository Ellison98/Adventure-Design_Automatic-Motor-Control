...
# 모터 각도 및 속도 초기화
angle = 90           # 서보 모터 초기 각도 (90)
offset = -12         # 서보 모터 보정 값 (-12)
...
# 모터 전진 및 후진 함수
def set_motor_direction(angle, speed):
    if speed > 0:  # 전진
        print("전진 중...")
        pca.channels[throttle_pin].duty_cycle = speed  # 속도 설정
    elif speed < 0:  # 후진
        print("후진 중...")
        pca.channels[throttle_pin].duty_cycle = abs(speed)  # 절대값으로 속도 설정
    else:  # 정지
        print("정지 중...")
        pca.channels[throttle_pin].duty_cycle = 0  # 속도 0으로 설정
...
while True:
    try:
        ...
        if data_dict.get('0', 0) < 200:  # 조종기를 밀었을 때 (전진)
            angle = 105  # 오른쪽으로 회전
            speed = d_speed  # 전진 속도
            set_motor_direction(angle, speed)  # 모터 전진
        elif data_dict.get('0', 0) > 200:  # 조종기를 당겼을 때 (후진)
            angle = 75  # 왼쪽으로 회전
            speed = -d_speed  # 후진 속도 (음수)
            set_motor_direction(angle, speed)  # 모터 후진
        else:
            # 기본 정지 동작
            angle = 90
            speed = 0  # 정지 속도
            set_motor_direction(angle, speed)  # 모터 정지
        ...
        pca.channels[servo_pin].duty_cycle = servo_angle(calc_angle)  # 서보 각도 설정
